...
def hello_pubsub(event):
    ...
    try:
        if _is_asset(msg):
            process_feeds(msg, cfg.slack_token, cfg.slack_channel)
            return
        if _is_gcs_iam_audit(msg):
            process_audit_logs(msg, cfg.slack_token, cfg.slack_channel)
            return
...
if __name__ == "__main__":
    ...
    path = sys.argv[1] if len(sys.argv) > 1 else "tests/fixtures/asset_project.json"
